...
# Portability guard: NEVER clobber a doc or hook the user has already customized (e.g. a
# translated AGENT_GUIDE or custom guard). If one exists, keep theirs and drop the shipped copy
# alongside as <name>.dist.md / <name>.dist.py so they can diff/merge deliberately.
...
for h in "$SCRIPT_DIR/hooks/"*.py; do
    [ -f "$h" ] || continue
    hname=$(basename "$h")
    dst="$HOME/.delegation_core/hooks/$hname"
    if [ -f "$dst" ]; then
        if ! cmp -s "$h" "$dst"; then
            cp -f "$h" "$HOME/.delegation_core/hooks/${hname%.py}.dist.py"
            echo "  • hook $hname modified locally: kept yours; shipped copy saved as ${hname%.py}.dist.py"
        fi
    else
        cp -f "$h" "$dst"
        echo "  • hook $hname installed"
    fi
done
echo "  ✓ Done."
echo ""
...
if [ -f "$HOME/.delegation_core/config.json" ]; then
    ...
    echo ""
    # Maintain background service registration on upgrades (DC-43)
    "$VENV/bin/delegation-core" service install >/dev/null 2>&1 || true
