...
# absolute path ("/shared/graphify-out"). Single source of truth in delegation_core.graph.paths
# (#1423); re-exported here as _GRAPHIFY_OUT for the existing call sites.
from delegation_core.graph.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`delegation_core.graph.llm._extraction_system`) or a Path to the
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`delegation_core.graph.watch._relativize_source_files` so cached
    ...
    :func:`delegation_core.graph.detect._to_relative_for_storage`.
    """
    try:
