...
from delegation_core.graph._minhash import MinHash, MinHashLSH
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from delegation_core.graph.llm import _call_llm
    except ImportError as exc:
        # F-038: previously this silent fallback hid the fact that `_call_llm`
        # didn't exist in `delegation_core.graph.llm` at all, so `--dedup-llm` was a no-op.
