...
from delegation_core.graph.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from delegation_core.graph.paths import GRAPHIFY_OUT, GRAPHIFY_OUT_NAME, out_path
...
def classify_file(path: Path) -> FileType | None:
    ...
    from delegation_core.graph.manifest_ingest import is_package_manifest_path
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`delegation_core.graph.watch._relativize_source_files` so the
