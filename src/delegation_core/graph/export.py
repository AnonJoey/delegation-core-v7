...
from delegation_core.graph.security import sanitize_label
from delegation_core.graph.analyze import _node_community_map
from delegation_core.graph.build import edge_data

from delegation_core.graph.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
...
def _yaml_str(s: str) -> str:
    ...
    See `delegation_core.graph.ingest._yaml_str` for the full rationale; duplicated here to
...
from delegation_core.graph.exporters.base import COMMUNITY_COLORS  # noqa: E402,F401

from delegation_core.graph.exporters.html import to_html  # noqa: E402,F401
...
def existing_graph_node_count(path: "str | Path"):
    ...
    if not p.exists():
        return None
    from delegation_core.graph.security import check_graph_file_size_cap
    try:
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
    ...
    from delegation_core.graph.paths import write_json_atomic
